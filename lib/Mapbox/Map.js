...
export default class Map extends React.Component {
    ...
    render() {
        const layerTypes = [RasterLayer, LineLayer, FillLayer, HeatmapLayer]
        let index = 0

        const childrenWithIndex = (element = this) => {
            return React.Children.map(element.props.children || [], child => {
                if (child === null || !child.props) {
                    return []
                }

                const children = childrenWithIndex(child)

                const props = {}
                if (layerTypes.includes(child.type) && [undefined, null].includes(child.props.index)) {
                    props.index = index++
                }
        ...
        const children = childrenWithIndex(this)

        return (
            <div className="map-container">
                <div className="map-container" ref={this.mapNodeRef} />
                <MapContext.Provider value={{ map: this.map, style: this.style }}>{children}</MapContext.Provider>
            </div>
        )
    }
}
